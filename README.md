
<img src="https://capsule-render.vercel.app/api?type=transparent&color=ffffff&height=150&section=header&text=Hey%20Everyone!&fontSize=90&fontAlign=50&&fontColor=3E55FA" />

---

### 👨‍💻 A Little Bit About Me and My Interests
```yaml
name: Aswin
located_in: Malappuram, Kerala, India
current_job: Aspiring FPGA Design Engineer
education:
  [
    "B.Tech in Electronics & Communication Engineering",
    "Diploma in Mechanical Engineering",
  ]

fields_of_interest:
  [
    "VLSI / FPGA Design",
    "Embedded Systems",
    "IoT",
    "Computer Vision",
  ]

technical_background:
  [
    "Verilog/FSM design on Xilinx Vivado",
    "ESP32 / STM32 embedded projects",
    "OpenCV-based computer vision",
  ]

currently_learning: ["Advanced VLSI/FPGA workflows"]
2026 Goals: ["Land an FPGA Design Engineer role", "Build 3+ new hardware projects"]
hobbies: ["Add your own here"]
```
  


---

### 🧰 Tech Stack & Tools

**Hardware Description / Digital Design**
![Verilog](https://img.shields.io/badge/-Verilog-blue?style=flat-square)
![Xilinx Vivado](https://img.shields.io/badge/-Xilinx%20Vivado-red?style=flat-square)

**Embedded Systems**
![ESP32](https://img.shields.io/badge/-ESP32-black?style=flat-square)
![STM32](https://img.shields.io/badge/-STM32-03234B?style=flat-square)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)
![FreeRTOS](https://img.shields.io/badge/-FreeRTOS-informational?style=flat-square)

**Languages**
![C](https://img.shields.io/badge/-C-A8B9CC?style=flat-square&logo=c&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

**IoT / Comms**
![MQTT](https://img.shields.io/badge/-MQTT-660066?style=flat-square)
![Blynk](https://img.shields.io/badge/-Blynk-03C3DB?style=flat-square)
![CAN Bus](https://img.shields.io/badge/-CAN%20Bus-orange?style=flat-square)

**CAD / Modeling**
![AutoCAD](https://img.shields.io/badge/-AutoCAD-E51050?style=flat-square)
![SolidWorks](https://img.shields.io/badge/-SolidWorks-red?style=flat-square)
![Fusion 360](https://img.shields.io/badge/-Fusion%20360-orange?style=flat-square)

**Computer Vision**
![OpenCV](https://img.shields.io/badge/-OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)

**Web**
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)

---

### 🚀 Featured Projects

- **🤖 Autonomous Manhole/Sewage Cleaning Robot** — Final year B.Tech project. ESP32-WROOM-32 + ESP32-CAM, OpenCV HSV-based detection, servo gripper, L298N motor driver, moisture sensor, suction pump.
- **🧺 SmartBasket** — ESP32-based smart waste basket with Blynk app control, HC-SR04 ultrasonic sensing, servo compression, buzzer/LED alerts, FreeRTOS deep-sleep firmware, and MQTT telemetry.
- **🚗 Smart Car Dashboard** — STM32F407 + CAN bus + DWIN LCD, communicating over MQTT.
- **🔢 SGPA Calculator** — A React app implementing the full KTU 2019 grading scheme.
- **🔺 Elevator Control System** — FSM-based digital design in Verilog, implemented on Xilinx Vivado.

> More details and code for each project are pinned below / linked in my repositories.

---

### 🔨 What I'm Currently Working On

<!-- Keep this section updated with whatever you're actively building -->
- 🔁 [PROJECT NAME] — one-line description of what it does and the stack you're using
- 🔁 [PROJECT NAME] — one-line description of what it does and the stack you're using

> I like to keep this list short and current — swap entries out as projects wrap up or new ones start.

---

### 📌 Pinned Repositories

<!--
Replace REPO_NAME and USERNAME below, or delete this section and let GitHub's native
pinned-repos feature handle it from your profile settings.
-->
<p align="center">
  <a href="https://github.com/YOUR_GITHUB_USERNAME/REPO_NAME">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=YOUR_GITHUB_USERNAME&repo=REPO_NAME&theme=default" />
  </a>
</p>

---
 <h2 align="center">Let's Connect and have a Chat! 💬</h2>
<p align="center">
  <a href="mailto:aswinxyz91@gmail.com">
    <img <img  height="70" hspace="10" alt="image" src="https://github.com/user-attachments/assets/e58ee6bf-3283-40c2-9ecb-e2793937303f" />
  </a>
  <a href="https://linkedin.com/in/aswinpln">
    <img height="65" hspace="10" alt="image" src="https://github.com/user-attachments/assets/f303e8d9-d323-46ff-b49f-9ce361cb8b3e" />
  </a>
  <a href="https://www.instagram.com/aswinnnnnnnn_">
    <img height="65" hspace="10" alt="image" src="https://github.com/user-attachments/assets/afb55320-2e33-45aa-b7e4-4d5223e72775" />
  </a>
</p>
---
### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?aswinportfolio&show_icons=true&theme=default" alt="GitHub stats" height="165"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?aswinportfolio&theme=default" alt="GitHub streak" height="165"/>
</p>


---

